"""This module had script for Renaming the generated query file to sql format and change that location"""
...
class SqlRename():
    ...
    def get_files_from_bucket(self):
        ...
        for object in my_bucket.objects.all():
            ...
            join_dir = os.path.join(os.getcwd(),os.path.basename(full_path))
            for fi in glob.glob("*.txt"):
                new_file=os.rename(fi, self.table_name + "sql")
            copy_source = {'Bucket': self.bucket_name, 'Key':object.key}
            # print(copy_source)
            s3_res.Object(self.bucket_name,self.database_name/new_file).copy_from(CopySource=copy_source)
